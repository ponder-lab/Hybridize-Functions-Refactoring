...
- `edu.cuny.hunter.hybridize.core` — analysis + refactoring engine. The interesting code is here:
	...
	- The `*.xml` files at the module root (`tensorflow.xml`, `pandas.xml`, `flask.xml`, `pytest.xml`, `click.xml`, `abseil.xml`, `functools.xml`) are Ariadne summary files. `update_summaries.sh` re-syncs them from a local `$HOME/ML` checkout.
