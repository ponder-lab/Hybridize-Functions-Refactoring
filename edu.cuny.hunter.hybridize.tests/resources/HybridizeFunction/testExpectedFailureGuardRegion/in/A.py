...
# and so excluded nothing at all. `pytest_guard` is the same shape under the pytest spelling,
# which shares the lowering because it is a property of `with` rather than of the manager.
...
with pytest.raises(tf.errors.InvalidArgumentError):
    pytest_guard(tf.ones((5,)))

assert pytest_guard(tf.ones((3, 3))).shape == (3, 3)
