...
A simpler alternative, dropping the `com.ibm.wala.ide` dependency entirely by inlining the two helper classes and switching `hybridize.target` to consume WALA from Maven Central, would eliminate this coordination burden permanently. Not yet attempted at the time of writing.
